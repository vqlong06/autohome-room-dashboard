...
Workflow `LongOS Production Smoke` được cấu hình kiểm tra health đầy đủ bốn lần mỗi giờ, lệch vào phút 07/22/37/52 để tránh giờ tròn đông tải, và có thể chạy thủ công trong tab **Actions**. Lịch `schedule` của GitHub Actions là best-effort: [GitHub có thể trì hoãn hoặc bỏ một số lượt khi tải cao](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule), nên workflow này không phải SLA giám sát 15 phút. Nếu cần cảnh báo đúng hạn, dùng thêm dịch vụ uptime monitor độc lập. Mỗi lượt đã được khởi chạy thử lại tối đa 3 lần khi có lỗi mạng tạm thời. Ngoài toàn bộ contract ở trên, job yêu cầu heartbeat `room_latest` không cũ quá 3 phút, mẫu history mới nhất không cũ quá 20 phút và các trạng thái ESP32, Wi-Fi, SHT30 đều online. Job ngay sau deployment vẫn chỉ kiểm tra contract web/cloud và đối chiếu exact commit revision; lỗi thiết bị sẽ do production health báo riêng.
