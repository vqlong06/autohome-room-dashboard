...
alter table public.health_metric_buckets
  add constraint health_metric_buckets_metric_check check (
    metric_key in ('steps', 'active_energy', 'sleep')
  ),
  ...
  add constraint health_metric_buckets_unit_check check (
    ...
    or (metric_key = 'sleep' and unit = 'minute')
  ),
...
create unique index if not exists health_metric_buckets_sleep_day_unique
on public.health_metric_buckets (user_id, metric_key, local_date, algorithm_version)
where metric_key = 'sleep';
...
begin
  ...
  if exists (
    ...
    where pg_catalog.jsonb_typeof(item.value) <> 'object'
       ...
       or not (
         (
           item.value ->> 'metric_key' = 'steps'
           and item.value ->> 'unit' = 'count'
           and item.value ->> 'provenance' = 'healthkit_statistics'
           and (item.value ->> 'value_integer')::bigint between 0 and 2000000
         )
         or (
           item.value ->> 'metric_key' = 'active_energy'
           and item.value ->> 'unit' = 'kcal'
           and item.value ->> 'provenance' = 'healthkit_statistics'
           and (item.value ->> 'value_integer')::bigint between 0 and 100000
         )
         or (
           item.value ->> 'metric_key' = 'sleep'
           and item.value ->> 'unit' = 'minute'
  ...
  select
    ...
    v_received_at
  from pg_catalog.jsonb_array_elements(p_buckets) item
  where item.value ->> 'metric_key' <> 'sleep'
  on conflict (
    user_id,
    metric_key,
    bucket_start,
  ...
  insert into public.health_metric_buckets as current_sleep (
    user_id,
    metric_key,
    bucket_start,
    bucket_end,
  ...
  do update set
    ...
    source_updated_at = excluded.source_updated_at,
    updated_at = v_received_at
  where excluded.source_updated_at >= current_sleep.source_updated_at;
